...
def infer_sql_types(dataframe_chunk: pd.DataFrame) -> dict[str, str]:
...
def ingest_csv(
    ...
    try:
        # Iterator-based loading avoids ingesting large CSVs fully into memory.
        chunks = pd.read_csv(resolved_path, chunksize=chunksize)
    ...
    target_table = table_name or _derive_table_name(resolved_path)
    schema = infer_sql_types(first_chunk)

    create_table_sql = _build_create_table_sql(schema_manager, target_table, schema)
    _execute_statement(query_executor, create_table_sql)

    rows_inserted = _insert_chunk(query_executor, target_table, first_chunk)
    for chunk in chunks:
        if chunk.empty:
            continue
        rows_inserted += _insert_chunk(query_executor, target_table, chunk)
...
def _build_create_table_sql(
    ...
    return f"CREATE TABLE IF NOT EXISTS {_quote_identifier(table_name)} ({columns});"


def _insert_chunk(query_executor: Any, table_name: str, chunk: pd.DataFrame) -> int:
    ...
    for row in rows:
        _execute_statement(query_executor, insert_sql, row)
